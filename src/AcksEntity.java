...
public class AcksEntity implements Runnable {
	...
	public void run() {
		try {
			while (!process.isTerminated()) {
				...
				if (message.isAck()) {
					if (process.containPendingMessage(message.getSenderId())) {
						...
						Log.receiveAckMessage(message.getSenderId(),
								process.getNumMessageReceivedAcks());

						if (checkIdleAndAllAcksReceived()) {
							// satisfy the termination condition for this
							// process
							...
							if (process.getId() == 1) {
								...
								process.incrementNumMessageReceivedAcks();
								Log.sendAckToParentAndDetachFromTree(parent
										.getId());
							}
						}
			...
			System.out.println("Ack thread terminated");
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public AcksEntity(InetAddress addr, int port) {
		...
		try {
			...
			System.out.println("Ack receive bind to " + port);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
